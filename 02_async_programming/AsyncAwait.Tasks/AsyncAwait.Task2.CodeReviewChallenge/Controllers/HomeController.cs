...
public class HomeController : Controller
{
    ...
    {
        ViewBag.Message = await _privacyDataService.GetPrivacyDataAsync(token);
        return View();
    }

    public async Task<IActionResult> Help(CancellationToken token)
    {
        ViewBag.RequestInfo = await _assistant.RequestAssistanceAsync("guest", token);
        return View();
    }
